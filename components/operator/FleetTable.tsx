...
//     const [deviceDialog, setDeviceDialog] = useState<{ open: boolean; busId: string | null; device?: BusDevice | null }>({
//         open: false,
//         busId: null,
//         device: null
//     })
...
//             console.log('✅ Fleet data received:', { busCount: data.buses?.length, pagination: data.pagination })
...
//     // ✅ LOAD DATA ON COMPONENT MOUNT AND SEARCH PARAMS CHANGE
//     useEffect(() => {
//         fetchFleetData()
//     }, [fetchFleetData])
...
//     // ✅ UTILITY FUNCTIONS
//     const getBusStatusConfig = useCallback((bus: BusWithDetails) => {
//         if (!bus.isActive) {
//             return {
...
//                 console.log('✅ Bus updated successfully:', updatedBus.busNumber)
//                 setEditingBus(null)
//                 setEditData({})
//                 await fetchFleetData()
...
//                 await fetchFleetData()
//                 alert(`Bus ${currentStatus ? 'deactivated' : 'activated'} successfully`)
//             } else {
...
//         setDeviceDialog({ open: true, busId, device })
//         if (device) {
//             setDeviceData({
...
//         setDeviceDialog({ open: false, busId: null, device: null })
...
//         try {
//             const isUpdate = !!deviceDialog.device
//             const url = isUpdate
...
//                 closeDeviceDialog()
//                 await fetchFleetData()
//                 alert(`Device ${isUpdate ? 'updated' : 'created'} successfully`)
//             } else {
//                 const errorData = await response.json().catch(() => ({ error: 'Unknown error' }))
//                 throw new Error(errorData.error || `Failed to ${isUpdate ? 'update' : 'create'} device`)
//             }
//         } catch (error) {
//             console.error(`❌ Error ${deviceDialog.device ? 'updating' : 'creating'} device:`, error)
//             const message = error instanceof Error ? error.message : `Failed to ${deviceDialog.device ? 'update' : 'create'} device`
//             alert(`Error: ${message}`)
//         } finally {
//             setActionLoading(null)
//         }
...
//                 await fetchFleetData()
//                 alert('GPS device removed successfully')
//             } else {
//                 const errorData = await response.json().catch(() => ({ error: 'Unknown error' }))
...
//                             <span>{deviceDialog.device ? 'Edit GPS Device' : 'Add GPS Device'}</span>
//                         </DialogTitle>
//                     </DialogHeader>
...
//                                 onChange={(e) => setDeviceData({ ...deviceData, isActive: e.target.checked })}
//                                 className="w-4 h-4 text-blue-600 rounded focus:ring-blue-500"
//                                 disabled={actionLoading === deviceDialog.busId}
//                             />
//                             <div>
//                                 <label className="font-medium text-gray-900">Active Device</label>
//                                 <p className="text-sm text-gray-600">
...
//                         <Button variant="outline" onClick={closeDeviceDialog} disabled={actionLoading === deviceDialog.busId}>
//                             Cancel
//                         </Button>
//                         <Button onClick={saveDevice} disabled={actionLoading === deviceDialog.busId}>
//                             {actionLoading === deviceDialog.busId ? (
//                                 <>
//                                     <Loader2 className="w-4 h-4 mr-2 animate-spin" />
//                                     {deviceDialog.device ? 'Updating...' : 'Creating...'}
//                                 </>
//                             ) : (
//                                 <>
//                                     <Save className="w-4 h-4 mr-2" />
//                                     {deviceDialog.device ? 'Update Device' : 'Create Device'}
//                                 </>
//                             )}
//                         </Button>
//                     </div>
...
//                         onChange={(e) => onEditDataChange({ ...editData, busNumber: e.target.value.toUpperCase() })}
...
//                             onChange={(e) => onEditDataChange({ ...editData, capacity: parseInt(e.target.value) || 0 })}
...
//             <td className="p-4">
//                 <Badge
//                     className={cn(statusConfig.color, "cursor-pointer hover:opacity-80 transition-opacity inline-flex items-center")}
...
//                                     Edit Device
//                                 </DropdownMenuItem>
//                                 <DropdownMenuItem onClick={onDeleteDevice} className="text-red-600">
...
//                         </div>
//                         <Button onClick={onAddDevice} variant="ghost" size="sm">
...
//                             <DropdownMenuItem onClick={() => router.push(`/operator/buses/${bus.id}`)} className="cursor-pointer">
...
//                             </DropdownMenuItem>
//                             {bus.device && (
//                                 <DropdownMenuItem onClick={() => router.push(`/operator/tracking?bus=${bus.id}`)} className="cursor-pointer">
...
//                             </DropdownMenuItem>
//                             <DropdownMenuItem className="text-red-600 cursor-pointer focus:text-red-600" onClick={onDelete}>
//                                 <Trash2 className="mr-2 h-4 w-4" />
//                                 Delete Bus
//                             </DropdownMenuItem>
//                         </DropdownMenuContent>
