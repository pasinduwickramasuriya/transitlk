...
//for auto hard refresh
    useEffect(() => {
        if (!sessionStorage.getItem('tracking-loaded')) {
            sessionStorage.setItem('tracking-loaded', 'true')
            window.location.reload()
        }
        
        // Clear flag when leaving page
        return () => {
            sessionStorage.removeItem('tracking-loaded')
        }
    }, [])
...
const fetchPositions = useCallback(async () => {
    ...
    try {
        ...
        // ✅ Silent handling - don't spam console with errors
        if (err.name === 'AbortError') {
            console.log('⏱️ Position fetch timed out')
            return
        }
        
        // ✅ Only log fetch failures once, don't set error state
 ...
 useEffect(() => {
    ...
    // ✅ Auto-refresh interval - simulates clicking the refresh button every 5 seconds
useEffect(() => {
    if (!isLiveTracking || !isInitialized) return

    const autoRefreshInterval = setInterval(() => {
        if (isMountedRef.current && isLiveTracking) {
            refreshAllData() // ✅ Calls the same function as the button
...
}, [isLiveTracking, isInitialized, refreshAllData])
    ...
    const getDeviceStatus = useCallback((device: Device) => {
        ...
        const now = new Date()
        const lastUpdate = new Date(device.lastSeen) // ✅ Use lastSeen (now synced with position)
        const minutesAgo = (now.getTime() - lastUpdate.getTime()) / (1000 * 60)

        if (minutesAgo <= 5) return 'online'   // ✅ Within 5 minutes = online
        if (minutesAgo <= 15) return 'stale'   // ✅ 5-15 minutes = stale
        return 'offline'                        // ✅ Over 15 minutes = offline
    }, [])
    ...
    // Count devices by actual status using the same logic as getDeviceStatus
    let onlineCount = 0
    let staleCount = 0
    let offlineCount = 0
    let inactiveCount = 0
    ...
    devices.forEach(device => {
        if (!device.isActive || !device.bus?.isActive) {
            inactiveCount++
            return
        }
    ...
    const activeBuses = devices.filter(device => device.isActive && device.bus?.isActive).length
    
    return {
        ...initialStats,
        totalBuses: devices.length,
        activeBuses,
        connectedDevices: onlineCount, // ✅ Shows actual online devices
        offlineBuses: offlineCount + staleCount, // Offline + stale combined
...
{/* ✅ Enhanced time and location display */}
{device.latestPosition && (
    <div className="mt-2 space-y-1">
        ...
        <p className="text-xs text-slate-500">
            🕐 {formatTimeAgo(device.lastSeen)} • {device.latestPosition.speed?.toFixed(1) || 0} km/h
        </p>
    </div>
)}
...
//             if (isMountedRef.current && err.name !== 'AbortError' && !error) {
...
//             }
//         }
//     }, [isLiveTracking, isInitialized, fetchPositions])
...
//     useEffect(() => {
//         if (isInitialized) {
//             const timeoutId = setTimeout(() => fetchDevices(), 300)
//             return () => clearTimeout(timeoutId)
//         }
//     }, [showOfflineDevices, fetchDevices, isInitialized])
...
//                 <CardContent className="flex items-center justify-center py-24">
//                     <div className="flex flex-col items-center gap-6 text-center max-w-md">
//                         <div className="w-20 h-20 bg-gradient-to-br from-orange-500 to-red-500 rounded-2xl flex items-center justify-center text-3xl">🗝️</div>
//                         <div className="space-y-4">
//                             <h3 className="text-2xl font-bold text-slate-800">Mapbox Token Required</h3>
//                             <p className="text-slate-600 leading-relaxed">Please add your Mapbox access token to enable live tracking map</p>
//                             <div className="bg-slate-100 p-4 rounded-xl border">
//                                 <code className="text-sm text-slate-700 font-mono">NEXT_PUBLIC_MAPBOX_ACCESS_TOKEN=your_token_here</code>
//                             </div>
//                             <Button onClick={() => window.open('https://account.mapbox.com/access-tokens/', '_blank')} className="bg-gradient-to-r from-orange-500 to-red-500 hover:from-orange-600 hover:to-red-600">
//                                 Get Mapbox Token
//                             </Button>
//                         </div>
//                     </div>
...
//                         <div className="flex items-center gap-3 px-4 py-2 bg-white/80 backdrop-blur-sm rounded-xl shadow-sm border border-white/20">
//                             <Radio className={cn("h-4 w-4", isLiveTracking ? "text-green-500" : "text-slate-400")} />
//                             <Switch checked={isLiveTracking} onCheckedChange={setIsLiveTracking} id="live-tracking" />
//                             <label htmlFor="live-tracking" className="text-sm font-medium cursor-pointer">Live Tracking</label>
//                             {isLiveTracking && <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse" />}
//                         </div>
//                         <div className="flex items-center gap-3 px-4 py-2 bg-white/80 backdrop-blur-sm rounded-xl shadow-sm border border-white/20">
//                             <Filter className="h-4 w-4 text-slate-600" />
//                             <Switch checked={showOfflineDevices} onCheckedChange={setShowOfflineDevices} id="show-offline" />
//                             <label htmlFor="show-offline" className="text-sm font-medium cursor-pointer">Show Offline</label>
//                         </div>
//                         <Button onClick={refreshAllData} variant="outline" size="sm" disabled={loading} className="bg-white/80 backdrop-blur-sm border-white/20 hover:bg-white/90">
//                             <RefreshCw className={cn("h-4 w-4 mr-2", (loading || isLoading) && "animate-spin")} />
//                             Refresh
//                         </Button>
//                     </div>
...
//                 <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
//                     <ModernStatsCard title="Total Buses" value={liveStats.totalBuses} icon={Bus} gradient="from-blue-500 to-cyan-500" bgGradient="from-blue-50 to-cyan-50" />
//                     <ModernStatsCard title="Online Buses" value={liveStats.connectedDevices} icon={Wifi} gradient="from-emerald-500 to-green-500" bgGradient="from-emerald-50 to-green-50" />
//                     <ModernStatsCard title="Offline Buses" value={liveStats.offlineBuses} icon={WifiOff} gradient="from-red-500 to-rose-500" bgGradient="from-red-50 to-rose-50" />
//                     <ModernStatsCard title="Recent Updates" value={liveStats.recentPositions} icon={Activity} gradient="from-orange-500 to-amber-500" bgGradient="from-orange-50 to-amber-50" />
//                     <ModernStatsCard title="Tracking Status" value={isLiveTracking ? 'Live' : 'Paused'} icon={isLiveTracking ? Play : Pause} gradient={isLiveTracking ? "from-green-500 to-emerald-500" : "from-slate-500 to-gray-500"} bgGradient={isLiveTracking ? "from-green-50 to-emerald-50" : "from-slate-50 to-gray-50"} isText={true} />
//                 </div>
...
//                     <Card className="lg:col-span-3 border-0 shadow-2xl overflow-hidden">
//                         <CardContent className="p-0">
...
//                                         {filteredDevices.filter(d => d.latestPosition).map(device => {
//                                             const status = getDeviceStatus(device)
...
//                                                 >
//                                                     <div className={cn("w-10 h-10 rounded-full shadow-xl flex items-center justify-center cursor-pointer transform hover:scale-125 transition-all", color)}>
//                                                         <Bus className="h-5 w-5 text-white" />
//                                                     </div>
//                                                 </Marker>
//                                             )
...
//                                             >
//                                                 <div className="p-4">
//                                                     <h3 className="font-bold text-lg">{selectedDevice.bus?.busNumber}</h3>
//                                                     <Badge className={cn("mb-2", getStatusColor(getDeviceStatus(selectedDevice)))}>
//                                                         {getDeviceStatus(selectedDevice).toUpperCase()}
//                                                     </Badge>
//                                                     <p className="text-sm">Speed: {selectedDevice.latestPosition.speed || 0} km/h</p>
//                                                     <p className="text-sm">Last: {formatTimeAgo(selectedDevice.lastSeen)}</p>
//                                                 </div>
//                                             </Popup>
//                                         )}
//                                     </Map>
...
//                                                 <div className="flex items-center justify-between">
//                                                     <div>
//                                                         <p className="font-bold">{device.bus?.busNumber || device.name}</p>
//                                                         <p className="text-xs text-slate-600">{device.bus?.busType} • {device.deviceId}</p>
//                                                     </div>
//                                                     <Badge className={cn("text-xs", getStatusColor(status))}>
//                                                         {status.toUpperCase()}
//                                                     </Badge>
...
//                     </div>
//                     <div className="flex-1 min-w-0">
//                         <p className={cn("font-bold text-slate-800 group-hover:text-slate-900 transition-colors", isText ? "text-xl" : "text-2xl")}>
//                             {typeof value === 'number' ? value.toLocaleString() : value}
//                         </p>
//                         <p className="text-sm text-slate-600 font-medium">{title}</p>
//                     </div>
